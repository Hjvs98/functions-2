...
const multiply = (num1, num2, callbackOne) => callbackOne(num1 * num2);
...
// Do not edit the code above.8709
...
const first = (arr, callbackTwo) => callbackTwo(arr[0]);
...
const last = (arr, callbackThree) => callbackThree(arr[arr.length - 1]);
...
const contains = (arr, name, callbackFour) => {
...
const getUserById = (arrOfObjects, id, cbSeven) => {
  for (let i = 0; i < arrOfObjects.length; i++) {
    if (arrOfObjects[i].id === id) {
      return cbSeven(arrOfObjects[i]);
    }
  }
};
